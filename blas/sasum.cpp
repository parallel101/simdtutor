...
// BEGIN CODE
float simd_sasum(int n, float const *X, int incX) {
    ...
    if (incX == 1) {
        while (X != Xe) {
            ...
            Sum1 = _mm256_add_ps(X1, Sum1);
            Sum2 = _mm256_add_ps(X2, Sum2);
            Sum1 = _mm256_add_ps(X3, Sum1);
            Sum2 = _mm256_add_ps(X4, Sum2);
            X += 32;
        }
    } else {
...
static void bench(benchmark::State &s) {
    ...
    for (auto _: s) {
        float res = simd_sasum(n, x.data(), 1);
...
TEST(MySuite, MyTest) {
    ...
    auto x = std::vector<float>(n);
    std::generate(x.begin(), x.end(), [uni = std::uniform_real_distribution<float>(), rng = std::mt19937()] () mutable { return uni(rng); });
    float res1 = simd_sasum(n, x.data(), 1);
    float res2 = cblas_sasum(n, x.data(), 1);
