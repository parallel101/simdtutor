...
static float multiScore(templateFeatSimd const *template_point_simd,
    ...
    auto $SumOfCoords = _mm256_add_ps(_mm256_setr_ps(-7, -6, -5, -4, -3, -2, -1, 0), _mm256_set1_ps(mbase));
    ...
    for(int m = mbase; m < ((template_feat_size + 7) & ~7); m += 8)
        {
        $SumOfCoords = _mm256_add_ps($SumOfCoords, $8);
        ...
        // 0 1+0 2+1+0 3+2+1+0
        $accum = _mm256_add_ps($accum, _mm256_set_m128(_mm256_castps256_ps128($accum), _mm_setzero_ps()));
        ...
        $PartialSum = _mm256_add_ps($PartialSum, _mm256_shuffle_ps($accum, $accum, 0xff));
        }
...
std::vector<matchResult> const &calSimilarity(const std::vector<templateFeat> &template_point,const std::vector<searchFeat> &search_point)
{
    ...
    for (int m = 0; m < template_point_simd.size() * 8; m += 8) {
        template_point_simd[m / 4].x = _mm256_setr_epi32(template_point[m].x, template_point[m + 1].x, template_point[m + 2].x, template_point[m + 3].x, template_point[m + 4].x, template_point[m + 4 + 1].x, template_point[m + 4 + 2].x, template_point[m + 4 + 3].x);
        template_point_simd[m / 4].y = _mm256_setr_epi32(template_point[m].y, template_point[m + 1].y, template_point[m + 2].y, template_point[m + 3].y, template_point[m + 4].y, template_point[m + 4 + 1].y, template_point[m + 4 + 2].y, template_point[m + 4 + 3].y);
        template_point_simd[m / 4].dx = _mm_setr_epi16(template_point[m].dx, template_point[m + 1].dx, template_point[m + 2].dx, template_point[m + 3].dx, template_point[m + 4].dx, template_point[m + 4 + 1].dx, template_point[m + 4 + 2].dx, template_point[m + 4 + 3].dx);
        template_point_simd[m / 4].dy = _mm_setr_epi16(template_point[m].dy, template_point[m + 1].dy, template_point[m + 2].dy, template_point[m + 3].dy, template_point[m + 4].dy, template_point[m + 4 + 1].dy, template_point[m + 4 + 2].dy, template_point[m + 4 + 3].dy);
        template_point_simd[m / 4].mag = _mm256_setr_ps(template_point[m].mag, template_point[m + 1].mag, template_point[m + 2].mag, template_point[m + 3].mag, template_point[m + 4].mag, template_point[m + 4 + 1].mag, template_point[m + 4 + 2].mag, template_point[m + 4 + 3].mag);
    }
#endif
    ...
    for(int i = 0; i < 1920; i++)
    {
        for(int j = 0; j < 1200; j += 8)
        {
            ...
            auto $SumOfCoords  = _mm256_set1_epi32(2);
            #else
            ...
            #ifdef USE_FIRSTUNROLL
            {
                ...
                if (!(curX < 0 || curX > 230)) {
                    ...
                    auto $cmp = _mm256_andnot_ps(_mm256_castsi256_ps($notok), _mm256_cmplt_ps($PartialSum, _mm256_set1_ps(thresholds[0])));
            ...
            for(;
                ...
                if (_mm256_movemask_epi8($loopterm) == 0xffffffff) {
                    break;
                }
                #else
                auto popcorn = __builtin_popcount(_mm256_movemask_epi8($loopterm));
                if (popcorn == 32 || (popcorn >= 28 && !(m & 7))) {
                    break;
                }
                #endif
            }
            ...
            #ifdef USE_MULTISCORE
            auto mask = _mm256_movemask_epi8($loopterm);
            if (mask != -1 && m < template_feat_size) {
